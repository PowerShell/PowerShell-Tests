...
Describe "Validation for GetPowerShell API Changes" {
    ...
    it "020 - Array of Arrays is passed back" {
        $sb = { write-output @(1,2,3),@(4,5,6),@(7,8,9) }
        $result = $sb.GetPowerShell().Invoke()|%{$_}
    ...
    it "037 - an array as a singleton is actually an array" {
        # since this is an array of one element, the write-output unravels
        # the arrayness, thus in order to test that we really have
        # an array of a single element, we need to protect from the
        # first unravelling
        $sb = { write-output @(,1)}
        $result = $sb.GetPowerShell().Invoke() | select-object -first 1
