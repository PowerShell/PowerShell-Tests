...
# Tests PowerShell batch execution
Describe -tags 'Innerloop', 'DRT' "TestBatchExecution" {
    ...
    Context "Remote Execution" {
        ...
        It "Should stop async operations correctly" {
            ...
            {
                $_.FullyQualifiedErrorId | should be "PSRemotingDataStructureException"
            }
        }
    }
...
Describe "PSDefaultParameters settings are effective" {
    BeforeAll {
...
Describe "PSDefaultPArameterValues settings are respected" {
    BeforeAll {
        $currentSettings = $PSDefaultParameterValues
        $PSDefaultParameterValues["Disabled"] = $true
    }
...
Describe -tags 'Innerloop', 'P1' "WinBlue_540698_NativeCommandRedirection" {
	...
	It "The 'network name cannot be found' error should be captured" {
		"$result2" | should match "network name cannot be found"
	}
    ...
    Context "using the PowerShell API captures error stream correctly" {
        BeforeAll {
            ...
            $ps.Invoke()
            $skip = $false
            if ($PSUICulture -ne 'en-US') { $skip = $true }
        }
        AfterAll {
            $ps.Dispose()
        }
        # en-us?
        It -skip:$skip "The 'network name cannot be found' error should be captured in the error stream" {
            [string]$ps.Streams.Error.Exception.Message | Should Match "network name cannot be found"
        }
        It "The first item should be an error record" {
